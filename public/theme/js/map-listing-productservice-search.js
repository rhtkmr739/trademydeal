(function ($) {
    ...
    function mainMap() {
	    ...
	    //  Map Infoboxes ------------------
        var allDataArray = localStorage.getItem("map-listing-productservice-search").split('~~');
        var finalFormatArrayList = [];
        allDataArray.forEach((data)=>{
            finalFormatArrayList.push(data.split('||'));
        });
        var locations = finalFormatArrayList;
	    //   Map Infoboxes end ------------------
      ...
      for (i = 0; i < locations.length; i++) {
        ...
        return function() {
             ...
             boxText.innerHTML = locationData(...locations[i][0].split('--'));
             ib.close();
             ib.open(map, overlay);
